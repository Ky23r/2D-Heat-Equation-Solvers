...
def gauss_seidel_with_5point_sor(
    length_x,
    length_y,
    nx,
    ny,
    ...
    while max_error > convergence_threshold:
        ...
        for i in range(1, ny - 1):
            for j in range(1, nx - 1):
                ...
                T_grid[i, j] = (1 - omega) * previous_T_grid[i, j] + omega * gs_update
    ...
    if plot:
        ...
        plt.colorbar()
        plt.title(
            "Steady-State Temperature Distribution in a 2D Plane\nUsing the Gauss-Seidel Iterative Method with 5-Point SOR"
        )
        plt.xlabel("X-axis")
        plt.ylabel("Y-axis")
        plt.tight_layout()
...
if __name__ == "__main__":
    _, _, _ = gauss_seidel_with_5point_sor(1, 1, 50, 50, 1e-4, 1.88, 400, 200, 300, 300)
